...
public class AuthDatabaseOperations {
  ...
  public AuthDatabaseOperations(DSLContext db) {
    this.db = db;
    this.SECONDS_VERIFICATION_EMAIL_VALID =
        Integer.valueOf(
            PropertiesLoader.getExpirationProperties()
                .getProperty("seconds_verification_email_valid"));
  ...
  public boolean isValidLogin(String username, String pass) {
    Optional<NoteUser> maybeUser =
        Optional.ofNullable(
            db.selectFrom(NOTE_USER)
  ...
  public void createNewUser(
    ...
    boolean usernameUsed =
        db.fetchExists(db.selectFrom(NOTE_USER).where(NOTE_USER.USER_NAME.eq(username)));
  ...
  private boolean isTokenDateValid(VerificationKeysRecord tokenResult) {
    Timestamp cutoffDate =
        Timestamp.from(Instant.now().minusSeconds(SECONDS_VERIFICATION_EMAIL_VALID));
  ...
  public void validateSecretKey(String secretKey) {
    VerificationKeysRecord veriKey =
        db.selectFrom(Tables.VERIFICATION_KEYS)
            .where(
                Tables.VERIFICATION_KEYS
                    .ID
                    .eq(secretKey)
    ...
    veriKey.store();
    db.update(Tables.NOTE_USER)
        .set(NOTE_USER.VERIFIED, 1)
